...
let IdentKeywords = ["continue", "goto"];
	...
	function token(type) {
		...
		if (type == "Number") {
			...
			if (src.substr(0, 2).toLowerCase() == "0x") {
				...
				ntype = "bin";
				if (parseInt(src.substr(2), 2) < 999999999999) src = parseInt(src.substr(2), 2);
			}
		}
		let tk = {
			Type: type,
...
function CreateLuaParser(text) {
	...
	function tableexpr(locals, upvals) {
		...
		while (peek().Source != "}") {
			...
			if (peek().Source == "[") {
				...
				(peek().Type == "Ident" || (peek().Type == "Keyword" && IdentKeywords.includes(peek().Source))) &&
				peek(1).Source == "="
			) {
				// Field
				let field = get();
	...
	function continuestat(locals, upvals) {
		if (peek(1).Source == "(") return [false, exprstat(locals, upvals)];
	...
	function gotostat() {
		let gotoKw = get();
		let labelKw;
		if (peek().Type == "Keyword") labelKw = expect("Keyword");
		else labelKw = expect("Ident");
...
function VisitAst(ast, visitors) {
	...
	for (var [visitorSubject, visitor] of Object.entries(visitors)) {
		if (visitorSubject === "VisitStat" || visitorSubject === "VisitExpr") continue;
...
function AddVariableInfo(ast) {
	...
	visitor.VariableExpr = function (expr) {
		expr.Variable = referenceVariable(expr.Token.Source, function (newName, saveStat = false) {
			if (saveStat && expr.IsStat) return;
...
function PrintAst(ast) {
	...
	let printExpr;
	let buffer = "";
	let getLeadingWhite = (tk) => (typeof tk?.LeadingWhite !== "string" ? " " : tk.LeadingWhite);
	...
	printStat = function (stat) {
		...
		if (stat.Type === "StatList" && stat.StatementList.length === 0) return;
...
function StripAst(ast) {
	...
	function joint(tokenA, tokenB, shit = false) {
		...
		let lastCh = (typeof tokenA.Source == "string" ? tokenA.Source : tokenA.Source.toString()).substr(
			tokenA.Source.length - 1,
			1,
		);
		...
		if (stat.Type == "StatList") {
			...
			stat.VarList.forEach((_var, index) => {
				...
				if (_var.Attribute) {
					...
					if (index + 1 == stat.VarList.length) dontStripEqualSign = true;
				}
			...
			if (stat.Token_Equals != null) {
				if (!dontStripEqualSign) stript(stat.Token_Equals);
				stat.ExprList.forEach((expr, index) => {
					stripExpr(expr);
					let comma = stat.Token_ExprCommaList[index];
					if (comma != null) {
...
function GetInnerExpression(stat) {
	let inner = stat;
	while (inner.Type === "ParenExpr") inner = inner.Expression;
	return inner;
}
	...
	switch (op.Source) {
		...
		case "#":
			if (arg.Type === "StringLiteral") {
				transformA2B(
					result,
					createLiteral("NumberLiteral", removeStringChars(arg.Token.Source).length, op.LeadingWhite),
				);
			}
			break;
	}
	...
	switch (op.Source) {
		...
		case "%":
			if (la.Type === "NumberLiteral" && ra.Type === "NumberLiteral") {
				...
				transformA2B(
					result,
					createLiteral("NumberLiteral", la1 - Math.floor(la1 / ra1) * ra1, la.GetFirstToken().LeadingWhite),
				);
			}
		...
		case "~=":
			if (la.Type !== "Unknown" && ra.Type !== "Unknown") {
				transformA2B(result, createLiteral("BooleanLiteral", !constantsEqual(la, ra), la.GetFirstToken().LeadingWhite));
			}
			break;
		case "==":
			if (la.Type !== "Unknown" && ra.Type !== "Unknown") {
		...
		case "and":
			if (la.Type !== "Unknown") {
				...
				//if (la === -1)
				//    return
				if (res) transformA2B(result, ra);
				else transformA2B(result, la);
			}
			break;
		case "or":
			if (la.Type !== "Unknown") {
				let res = isTruthful(la);
				// if la !== -1
				if (res) transformA2B(result, la);
				else transformA2B(result, ra);
			}
			break;
	}
}
...
function ValueTracking(ast) {
	...
	function assign(variable) {
		if (variable.Type == "VariableExpr") {
			...
			if (variables[variable.Token.Source]) variables[variable.Token.Source].written = true;
		} // else if Global
		else {
			// visit
		}
...
function SolveSimpleMath(ast, variables, globals) {
	...
	analyze = (node) => {
		...
		switch (node.Type) {
			...
			case "ParenExpr":
				...
				if (innerExpr.Type !== "Unknown") result = innerExpr;
				break;
			case "UnopExpr":
				let arg = analyze(node.Rhs);
				if (arg.Type !== "Unknown") foldUnary(result, node.Token_Op, arg);
				break;
			case "BinopExpr":
				let la = analyze(node.Lhs);
				let ra = analyze(node.Rhs);

				if (la.Type !== "Unknown") foldBinary(result, node.Token_Op, la, ra);
	...
	visitor.UnopExpr = (expr) => {
		...
		if (result.Type !== "Unknown") transformA2B(expr, result);
	};
	...
	visitor.IfStat = (stat) => {
		...
		if (result.Type !== "Unknown") transformA2B(stat, result);
	};
